...
require(['jquery', 'core/modal_factory', 'core/modal_events', 'core/ajax', 'core/notification'], function($, ModalFactory, ModalEvents, Ajax, Notification) {
   ...
   $("#email").blur(function() {
      ...
      let request = {
          methodname: 'local_participant_check_email',
          args: {email: email},
      };
   ...
   var trigger = $('.local_participant_delete_button');
   ModalFactory.create({
      type: ModalFactory.types.SAVE_CANCEL,
      title: 'Delete Participant',
      body: '<p>Are you sure to delete this participant ?</p>',
      preShowCallback: function(triggerElement, modal) {
         ...
         modal.params = {'id': id};
         modal.setSaveButtonText('Delete Participant');
      },
      large: true
   }, trigger)
   .done(function(modal) {
      ...
      modal.getRoot().on(ModalEvents.save, function(e) {
         ...
         spinner.show();
         let request = {
            methodname: 'local_participant_delete_participant',
            args: modal.params,
         };
         Ajax.call([request])[0].done(function(data) {
            if (data === true) {
