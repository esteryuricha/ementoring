...
class manager {
    ...
    public function insert_sponsor(int $suspended, string $firstname, string $lastname, string $email, string $password, string $picture): bool {
        global $DB;
        ...
        try {
            ...
            $recordtoroleassignments->roleid = 4;
    ...
    function get_sponsors(): array
    {
        ...
        $sql = "SELECT ROW_NUMBER() OVER(order by u.id) AS num,
                ...
                ON u.id = ra.userid 
                WHERE ra.roleid=4 and contextid=1 and u.id!=$USER->id";
        return $DB->get_records_sql($sql);
    }

    function update_sponsor(int $suspended, string $firstname, string $lastname, string $email, string $password, string $picture): bool {
        global $DB, $SESSION;
